...
class Animal:
    ...
    def evmulas(self):
        self.szaporodasCheck()
        self.eletkor += 1
        if self.eletkor >= self.maxelet:
            self.meghal()
        if self.szules_ota >= 0:
            self.szules_ota += 1
    ...
    def szaporodasCheck(self):
        if self.ehseg == 0:
            szaporodasterulet = map.get_characters_around(self.x, self.y)
            ragadozo_index = [index for index, (karakter, _) in enumerate(szaporodasterulet) if karakter == 'R']
            novenyevo_index = [index for index, (karakter, _) in enumerate(szaporodasterulet) if karakter == 'N']
            terulet_index = [index for index, (karakter, _) in enumerate(szaporodasterulet) if karakter == '.']
            if self.type == "N":
                if novenyevo_index:
                    if terulet_index:
                        if self.szules_ota >= 2:
                            self.szules_ota = 0
                            random_terulet = random.choice(terulet_index)
                            x, y = szaporodasterulet[random_terulet][1][0], szaporodasterulet[random_terulet][1][1]
                            Animal("N", x, y, map)

            elif self.type == "R":
                if ragadozo_index:
                    if terulet_index:   
                        if self.szules_ota >= 3:
                            self.szules_ota = 0
                            random_terulet = random.choice(terulet_index)
                            x, y = szaporodasterulet[random_terulet][1][0], szaporodasterulet[random_terulet][1][1]
                            Animal("R", x, y, map)
